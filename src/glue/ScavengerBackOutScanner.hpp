...
public:
	...
	scanAllSlots(MM_EnvironmentBase *env)
	{
		...
		for (uintptr_t i = 0; i < omrVM->valueStackSize; ++i) {
			volatile uintptr_t *slot = &omrVM->valueStack[i];
			if (*slot != 0 && (*slot & 1) == 0) {
				_scavenger->backOutFixSlotWithoutCompression((volatile omrobjectptr_t *)slot);
			}
		}
		J9HashTableState state;
		ToyObjectEntry *objectEntry = (ToyObjectEntry *)hashTableStartDo(omrVM->objectTable, &state);
		while (NULL != objectEntry) {
			...
			objectEntry = (ToyObjectEntry *)hashTableNextDo(&state);
		}
